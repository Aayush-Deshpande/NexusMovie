...
with tab1:
    ...
    if eval_stats:
        ...
        st.markdown("### Automatic K Selection")
        col_k1, col_k2 = st.columns([1, 2])
        with col_k1:
            st.markdown(f"**Optimal K:** `{eval_stats.get('num_clusters', 25)}`")
            st.write(f"- Silhouette Best: **{eval_stats.get('silhouette_best', 0):.4f}**")
        ...
        with col_k2:
            if os.path.exists('eda_assets/k_sweep.png'):
                st.image("eda_assets/k_sweep.png", use_container_width=True, caption="Elbow vs Silhouette Analysis Sweep")
